...
cpu主要完成的就是从程序获取指令并执行计算，负责的就是解释和运行最终转换成机器语言的内容。一段程序通常经过虚拟机的编译后，转换成二进制的字节码文件（可以进一步编译成二进制的机器代码文件），运行时会分配内存（内存的作用就是存放cpu的运算数据），然后cpu从内存中取出指令，执行指令。

所以从上面的描述中，也可以知道，cpu主要由两部分组成：控制单元（从内存中取出指令并执行--执行就是靠逻辑单元完成）、算数逻辑单元（处理算数和逻辑运算）。

通常现代计算机看起来是会有多个程序同时执行的（并行(多cpu)或者并发(单cpu)），每个程序执行的时候都需要操作系统分配资源（cpu和内存），这就是进程的概念——资源分配的基本单位，是一个动态的概念，就是程序的一次执行过程；执行过程的状态和申请的资源要记录在一个数据结构中进程控制块PCB（会记录程序执行到哪里，还有当前进程标识符pid、进程状态(阻塞、运行、就绪)、进程通讯用的数据结构等），所以进程包括了代码（程序段指针-当前进程的程序在内存的什么地方）、数据（数据段指针-当前进程的数据在内存的什么地方）、PCB；因为进程是资源分配的单位（包括内存地址空间），因此各进程拥有的内存地址空间相互独立，但是进程间的消息交换又是必须的，所以有了进程通信。关于进程通信，通常的实现方式有信号量（常用于进程同步(本质上就是存在于pcb的一个属性，属性值是一个数)，有p操作(-1)v操作(+1)）、管道（半双工，读写文件的思想(当管道写满时，写进程的write()系统调用将被阻塞，等待读进程将数据取走。当读进程将数据全部取走后，管道变空，此时读进程的read()系统调用将被阻塞。)）、消息队列（用一个队列做缓冲实现了异步性）、共享内存（申请一段多进程都可以操作的内存）；

线程是cpu调度执行的基本单元，可以理解为更轻量级的进程；主要是为了在一个进程里可以并发处理各种任务（进程是程序的一次执行过程，在一次执行中可能会做很多操作（聊天、发文件），这些操作就是各种线程）；线程是依附于进程的，可以与同属一个进程的其他线程共享进程拥有的全部资源；

进程和线程的区别：每个进程都有独立的代码和数据空间（程序上下文），进程之间的切换会有较大的开销；线程共享代码和数据空间，每个线程都有自己独立的运行栈和程序计数器（PC），线程之间切换的开销小。说起进程切换，是指回收当前运行程序对cpu的控制权，并将控制权转给新调度的就绪进程。

主要是靠中断来驱动，当中断发生时，cpu的执行模式会从用户态转到内核态，来处理中断信号，由进程调度程序选一个就绪进程（新程序）投入运行，进入用户态；这时候旧进程的状态变为就绪态或者阻塞态，新进程处于运行态
